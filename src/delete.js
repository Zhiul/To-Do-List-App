...
export const initializeDeleteEventListeners = (() => {
  document.addEventListener('click', (event) => {
    if (event.target.matches('.delete-project')) {
      ...
      if (deleteProjectOption.closest('.project-item')) {
        projectIndex = parseInt(
          deleteProjectOption.closest('.project-item').dataset.project,
        );
      } else {
        projectIndex = parseInt(
      ...
      if (projectIndex !== 0) {
        const deleteProjectModalTitle = deleteProjectModal.querySelector('.item-title');

        deleteProjectModal.dataset.project = projectIndex;
        deleteProjectModalTitle.textContent = toDoProjects.projects[projectIndex].title;
      }
    }
  });
  ...
  document.addEventListener('click', (event) => {
    if (event.target.matches('.delete-task')) {
      ...
      const projectIndex = deleteTaskButton.closest('[data-project]').dataset.project;
      ...
      if (completedTask) {
        ...
        deleteTaskModalTitle.textContent = toDoProjects.projects[projectIndex].sections[
          sectionIndex
        ].completedTasks[taskIndex].title;
      } else {
        deleteTaskModalTitle.textContent = toDoProjects.projects[projectIndex].sections[sectionIndex].tasks[
          taskIndex
        ].title;
      }
    }
  ...
  document.addEventListener('click', (event) => {
    if (event.target.matches('.delete-comment')) {
      ...
      const deleteTaskModalTitle = deleteCommentModal.querySelector('.item-title');
  ...
  document.addEventListener('click', (event) => {
    if (event.target.matches('.delete-section')) {
      ...
      const deleteSectionModalTitle = deleteSectionModal.querySelector('.item-title');
  ...
  deleteButton.addEventListener('click', () => {
    ...
    const actualProjectIndex = document.querySelector('.main-content.enabled')
      .dataset.project;

    const deleteProjectMode = deleteModal.dataset.project
      && !deleteModal.dataset.section
      && !deleteModal.dataset.taskIndex
      && !deleteModal.dataset.commentIndex;
    const deleteTaskMode = deleteModal.dataset.project && deleteModal.dataset.taskIndex;
    const deleteProjectCommentMode = deleteModal.dataset.project && deleteModal.dataset.commentIndex;
    const deleteSectionMode = deleteModal.dataset.project && deleteModal.dataset.section;
    ...
    if (deleteProjectMode) {
      ...
      projectElements.forEach((projectElement) => {
        if (!projectElement.matches('.delete-modal')) {
      ...
      const commentsNumber = toDoProjects.projects[projectIndex].comments.length;
